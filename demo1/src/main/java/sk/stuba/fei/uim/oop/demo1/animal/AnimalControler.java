...
public class AnimalControler {
    ...
    @GetMapping("/{name}")
    public List<AnimalResponse> getAllAnimalsByNames(@PathVariable("name") String name){
        return  this.service.getAllByNames(name).stream().map(AnimalResponse::new).collect(Collectors.toList());
    }
